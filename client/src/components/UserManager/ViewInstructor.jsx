			...
			<TableRow className={classes.root}>
				...
				<TableCell>
					<Button
						...
						onClick={() => {
							axios.delete(`/api/auth/user/${row.id}`).then((res) => {
								console.log(res);
							});
						}}
					>
...
export const ViewInstructor = () => {
	const [instructor, setInstructor] = React.useState(null);

	const history = useHistory();
	useEffect(() => {
		axios.get("/api/auth/user/instructor").then((response) => {
			...
			<TableContainer>
				...
				<div style={{ display: "flex", justifyContent: "space-between" }}>
					...
					<Button
						className='button button-primary'
						variant='contained'
						onClick={() => history.push("/users/create/instructor")}
					>
						Add new Instructor
					</Button>
				</div>
				...
				<Table aria-label='collapsible table'>
					...
					<TableBody>
						{instructor &&
							instructor.map((row) => <Row key={row.name} row={row} />)}
					</TableBody>
				</Table>
			</TableContainer>
		</Container>
